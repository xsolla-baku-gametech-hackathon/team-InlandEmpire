# Checklist

Tick a box when the thing runs, not when the code is written. Claude updates
this file when it has verified the condition. Owner initials in brackets.

## Before kickoff (accounts, installs, reading only)

- [x] Team repo created, `dev` branch exists, this folder is on it [C]
- [ ] Everyone has `claude` running from the repo root
- [x] Rust stable, `cargo tauri --version` prints [B]
- [x] `cargo tauri dev` opens a blank window on the game laptop [B]
- [x] Xsolla Publisher Account, sandbox project, project id noted [A]
- [x] Server API key created in Publisher Account [A]
- [x] Item `gems_500` in the catalogue, price 4.99 USD [A]
- [x] The `curl` in `docs/xsolla.md` returns a token and an order_id [A]
- [x] Test bank card numbers copied from Xsolla docs [A]
- [x] Arduino IDE flashes the ESP32, RC522 prints a card UID [H]
- [x] Two NFC cards, UIDs written down [H]
- [x] Skills installed in the repo: `npx skills add apollographql/skills@rust-best-practices` and `npx skills add nodnarbnitram/claude-code-extensions@tauri-v2` [C]
- [ ] Deck template opened, problem slide drafted [P]

## Hour 1

- [x] `tappad-protocol` builds, tests pass [C]
- [x] CI green on `dev` [C]
- [x] `POST /purchase` with mock returns `approved` for Gold [A]
- [x] `POST /purchase` with mock returns `declined limit_exceeded` for Starter [A]
- [x] Tauri window shows shop and gem counter [B]
- [x] Firmware prints `{"event":"tap","uid":"..."}` per tap [H]
- [x] Mentor asked about Tokenization, answer written in README [A]

## Hours 2 to 3

- [x] `tappad-bridge --fake` broadcasts taps on `:8765` [C]
- [x] Game shows "tap detected" from the fake bridge [B]
- [x] Game calls `POST /purchase`, shows declined text [B]
- [x] Xsolla client creates a real sandbox order [A]
- [x] Bridge reads the real serial port [C]

## Checkpoint 13:00, tag `v0.1.0-mock`

- [x] Fake tap gives gems end to end with the mock provider
- [x] `dev` merged to `main`, tagged [C]

## Hours 4 to 5

- [ ] iframe shows the sandbox checkout inside the game [B]
- [x] Polling flips to paid, gems granted [B]
- [ ] Real pad tap starts a real sandbox purchase [A B C H]
- [x] Integration test: fake tap to gems, mock provider [C]

## Checkpoint 15:30, tag `v0.2.0-sandbox`

- [ ] Real Xsolla sandbox payment through the game
- [ ] `dev` merged to `main`, tagged [C]

## Hours 6 to 7

- [ ] Backup video recorded, two taps, under one minute [P]
- [x] README says what is real and what is mocked, and it is true [C]
- [ ] `docs/protocol.md` matches the code [C]
- [x] Clippy clean on every crate, no `unwrap` outside tests [all]
- [ ] Each crate reviewed by someone who did not write it
- [x] Bonus only: `verify_signature` module with tests, no route [A]

## Checkpoint 18:00, tag `v0.3.0-demo`

- [ ] Gold pays, Starter declined, on the real pad
- [ ] `dev` merged to `main`, tagged [C]

## Sept 11

- [ ] 10:00 full rehearsal on the presentation laptop, real pad
- [ ] 12:00 freeze, tag `v1.0.0`, nobody pushes after this [C]
- [x] 13:00 decision: real Xsolla or mock for the demo
- [ ] 13:30 deck submitted to organisers [P]
- [x] Presentation laptop: server, bridge, game start with one script
- [ ] Spare pad and spare cards in the bag [H]
